...
df_ = pd.read_csv("Python/Recommendation_Systems/armut_data.csv")
df = df_.copy()
df.info()
df.shape
df.describe().T
...
df["Hizmet"] = df["ServiceId"].apply(lambda x: str(x)) + "_" + df["CategoryId"].apply(lambda x: str(x))
...
df["New_date"] = df["CreateDate"].apply(lambda x: x[:7])
df["NEW_DATE"] = df["CreateDate"].dt.strftime("%Y-%m")

df["Sepet_Id"] = df["UserId"].astype(str) + "_" + df["New_date"]
...
df_pivot = df.pivot_table(index="Sepet_Id", columns="Hizmet", values="ServiceId", aggfunc="sum").fillna(0). \
