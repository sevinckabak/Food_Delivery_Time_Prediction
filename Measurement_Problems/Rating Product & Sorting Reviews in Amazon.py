...
df = pd.read_csv(r"Python/Measurement_Problems/Rating Product&SortingReviewsinAmazon/amazon_review.csv")
...
today_date = df["reviewTime"].max() + pd.Timedelta(days=2)
df["day_diff"].max()
...
# 4.6987161061560725
# Yakın zamanda yapılan yorumların ortalaması daha yüksek.
...
df.head()
df.sort_values("helpful_no", ascending=False).head(10)
