...
# skompiler excel, Python ve SQL formunda çıktılar alınacak.
# model validasyonu daha kapsamlı öğrenilecek.
...
# y_prob; olasılık değeri


# Confusion matrix
print(classification_report(y, y_pred))
# yüzde yüz dopru tahmin çıktısı
...
# cross_validate metodu cart model'de yer alan fit(X, y)'yi görmez.
# kendisi veriyi parçalara bölerek model kurar.
# bu sebeple fit(X, y) yazmaya da biliriz.
...
# amaç; model doğrulama. başarının yüksek/düşük olması değil.
# veri ön işleme & hiperparametre optimizasyonu (ek olarak dengesiz veri yaklaşımları) ile model başarısını arttırma
# dengesiz veri yaklaşımları; gözlem birimlerinin dağılımlarının birbirlerine yaklaştırılmaya çalışılması.
...
# bu parametreler neye göre verilmeli; ön tanımlı değer baz alınır.
# ön tanımlı değerin etrafındaki değerleri deneyebiliriz.
...
# verbose; raporlama türünü ifade eder. Seviyeleri vardır (1, 2, 3 vb)
# Hiperparametre optimizasyonu; train seti ile yapılabilir veya tüm veri ile yapılabilir.
# Genelde tüm veri seti ile yapılır.

cart_best_grid.best_params_
# scoring=accuracy; {'max_depth': 5, 'min_samples_split': 4}
# scoring=f1; {'max_depth': 4, 'min_samples_split': 2}
# scoring=roc_auc; {'max_depth': 5, 'min_samples_split': 19}

cart_best_grid.best_score_
# scoring=accuracy; 0.75
# scoring=f1; 0.63
# scoring=roc_auc; 0.80


# GridSearchCV fonksiyonu scoring parametresi ön tanımlı değeri accuracy'dir.
# genelde ön tanımlı değer kullanılır.


# ağaç yöntemlerinde standartlaştırmaya gerek yok.

# GridSearchCV ile ilerlenebilir. Biz final modeli kullanacağız.
...
def plot_importance(model, features, num=len(X), save=False):
    # num argümanı biçimlendirilen değişken sayısı kadar görselleştirme yapar.
    feature_imp = pd.DataFrame({'Value': model.feature_importances_, 'Feature': features.columns})
    # featureların ve değerlerin isimlerini dataframede tutuyor.
    plt.figure(figsize=(10, 10))
    sns.set(font_scale=1)
    sns.barplot(x="Value", y="Feature", data=feature_imp.sort_values(by="Value",
                                                                     ascending=False)[0:num])
    # değişken görselleştirir büyükten küçüğe sıralar.
    plt.title('Features')
    plt.tight_layout()
    plt.show()
    # True dersek ilgili görseli kaydeder.
    if save:
...
# max_depth'in değerini range(1, 11) aralığında döndürerek 10 katlı çapraz doğrulama yapar ve hata çıktılarını arrayler olarak verir.
...
# Validation yerine test de denebilir. aynı şeyi ifade eder.
...
# burada tek başına max_depth'e bakılır.
# Fakat iki parametre değiştirdik ve aslında parametrelerin optimumunu bulduk.
...
# Özel ödev: max_depth'i 3'e indirip oradaki türm adımları tek tek hesaplama yap.
...
# bu kodlarla fonksiyonlaştırılabilir.

print(skompile(cart_final.predict).to('sqlalchemy/sqlite'))

print(skompile(cart_final.predict).to('excel'))

# canlı ortamda modeli kullanmak için bu kuralları kullanabiliriz.
...
# predict metodu kullanıldığından x dataframe'e çevirilir.
